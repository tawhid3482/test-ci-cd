...
const userLogin = async (payload: Partial<{ email: string; password: string; role: string }>) => {
...
const resetPassword = async (payload: { email: string; newPassword: string }) => {
  ...
  if (user.status ==="SUSPENDED") throw { status: 401, message: "User already deleted" };
...
const forgotPassword = async (email: string) => {
  ...
  if (!user) throw { status: httpStatus.BAD_REQUEST, message: "User does not exist" };
  if (["SUSPENDED", "INACTIVE"].includes(user.status)) throw { status: httpStatus.BAD_REQUEST, message: `User is ${user.isActive}` };
...
const changePassword = async (oldPassword: string, newPassword: string, decodedToken: JwtPayload) => {
  const user = await prisma.user.findUnique({ where: { id: decodedToken.id as string } });
  if (!user) throw { status: httpStatus.UNAUTHORIZED, message: "User not found" };

  const isOldPasswordMatch = await bcrypt.compare(oldPassword, user.password);
  if (!isOldPasswordMatch) throw { status: httpStatus.UNAUTHORIZED, message: "Old password does not match" };
